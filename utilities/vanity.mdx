...
- [ld | logDelay]="5S"
- [o | outDir] (required; no default; example: '.keys')
- [sv | sigVerify]=false
...
./sava-vanity/genKeys.sh --prefix="abc" --outDir=".keys" --password

# Non-interactive: read the password from an existing environment variable.
export MY_VANITY_PASSWORD="..."
./sava-vanity/genKeys.sh --prefix="abc" --outDir=".keys" --passwordEnv=MY_VANITY_PASSWORD
```
...
./sava-vanity/genKeys.sh --prefix="abc" --outDir=".keys" --password --kdf=argon2id \
  --kdfMemoryKB=262144 --kdfParallelism=4 --kdfIterations=3

# Opt out of Argon2id and customize only the PBKDF2 iteration count.
./sava-vanity/genKeys.sh --prefix="abc" --outDir=".keys" --password --kdf=pbkdf2 --kdfIterations=600000
```
