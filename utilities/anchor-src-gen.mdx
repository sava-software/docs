...
description: "Generates utilities for interacting with onchain programs and accounts."
---
...
- `program` (string, required unless `idlURL` or `idlFile` is provided)
  - Base58-encoded Solana program address (PublicKey). Used to compute the on-chain IDL address and to fetch the IDL
    when not using `idlURL` or `idlFile`.
- `idlURL` (string, optional)
  - HTTP(S) URL to fetch the program's IDL JSON. When provided, the generator fetches the IDL from this URL instead of
    the chain or file.
- `idlFile` (string, optional)
