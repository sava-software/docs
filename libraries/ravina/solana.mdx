...
```java
RpcCaller rpcCaller; // see docs above
...
Provides functionality for tracking transaction confirmation status via asynchronous and polling signature status RPC calls. 
...
var txMonitorConfig = TxMonitorConfig.parseConfig(txMonitorJsonConfig);
...
After sending a transaction, a confirmed state signature status Web Socket subscription is made, which expires based on the Transaction Monitor Service's configuration. 
...
After which one final call is made with transaction history enabled to ensure the transaction did not land but was expired from the local history node cache.

While waiting for expiration the transaction may optionally be re-sent based on Transaction Monitor Service's configuration.
...
var transactionResult = instructionService.processInstructions(
    ...
    true, // Continue to re-send the transaction based on the Transaction Monitor Service's configuration.
