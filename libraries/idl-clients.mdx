...
Clients are generated from each program's IDL, providing typed instruction builders and
account (de)serialization, plus hand-written convenience clients where useful.

| Module               | Programs                                                                                                                                                                       |
|----------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `idl-clients-spl`    | Token, Token-2022, Associated Token Account, System, Stake, Stake Pool, Address Lookup Table, Memo, Compute Budget, signature verification precompiles, Attestation Service |
| `idl-clients-bundle` | Jupiter, Kamino, Marginfi, Marinade, Metaplex, Meteora, Orca, Phoenix, Squads, CCTP, Loopscale, Neutral Trade, Pyth, Switchboard, Wormhole                                     |

## Build Configuration

```kotlin build.gradle.kts
dependencies {
  implementation("software.sava:idl-clients-spl:$VERSION")
  implementation("software.sava:idl-clients-bundle:$VERSION")
}
```
...
<a id="typical-transaction-flow"></a>

### Typical Legacy Transaction Flow
...
`TxBuilder` currently builds only v1 transactions and accepts the same generated
`Instruction` objects.
See the [transaction guide](/guides/transactions) for v1 construction and signing. The
mainnet examples on this page continue to use the supported legacy transaction factories.

### Transfer SOL

```java
// splAccountClient, owner & feePayer from "Create a Program Client" above.
...
try (var httpClient = HttpClient.newHttpClient()) {
  ...
  var createStakeAccountIx = SystemProgram.createAccountWithSeed(
      ...
      stakeAuthority,     // base account, must sign
      stakeAuthority,     // base key the seed derives from
      seed,
...
```java
// rpcClient, owner & feePayer from the examples above.
var solanaAccounts = SolanaAccounts.MAIN_NET;
var splClient = SPLClient.createClient(solanaAccounts);
var splAccountClient = splClient.createAccountClient(owner, feePayer);
...
try (var httpClient = HttpClient.newHttpClient()) {
  ...
  var base64Encoded = transaction.base64EncodeToString();
  var sig = rpcClient.sendTransaction(base64Encoded).join();
  System.out.println("https://explorer.solana.com/tx/" + sig);
